...
class AddedItemsWithPrinterAlignment extends StatefulWidget {
...
//HowManyIsAdded
  ...
  AddedItemsWithPrinterAlignment(
  ...
  @override
  _AddedItemsWithPrinterAlignmentState createState() =>
      _AddedItemsWithPrinterAlignmentState();
}

class _AddedItemsWithPrinterAlignmentState
    extends State<AddedItemsWithPrinterAlignment> {
...
//ThisIsTheStringToUpdateInPlaystore
  ...
  List<int> kotBytes = [];
  // num lastMillisecondCouldntConnectMessageWasShown = 0;
    ...
    if (bluetoothOnTrueOrOffFalse) {
      ...
      for (var device in _devices) {
        ...
        if (devicesCount == _devices.length &&
          ...
          show('Couldn\'t Connect. Please check Printers');
          printingOver = true;
        }
      }
    } else {
...
//             print(now);
//             print('lastMillisecondCouldntConnectMessageWasShown');
//             print(lastMillisecondCouldntConnectMessageWasShown);
//
// //ThisWillCheckWhetherItHasBeenTenSecondsSinceWeLastShowedTheCouldntConnectMessage
//
//             if ((((now.hour * 3600000) +
//                         (now.minute * 60000) +
//                         (now.second * 1000) +
//                         now.millisecond)) -
//                     lastMillisecondCouldntConnectMessageWasShown >
//                 10000) {
//               lastMillisecondCouldntConnectMessageWasShown =
//                   ((now.hour * 3600000) +
//                       (now.minute * 60000) +
//                       (now.second * 1000) +
//                       now.millisecond);
//               bluetooth.disconnect().whenComplete(() {
...
//             }
  ...
  void printKOTBytesThroughBluetooth() {
    print('start of inside printKotBytesThroughBluetooth');
    if (_connected) {
      bluetooth.isConnected.then((isConnected) {
        print('came inside bluetooth isConnected');
        if (isConnected == true) {
          ...
          _disconnectForKOTBytesBluetoothPrint();
          //bluetooth.drawerPin2(); // or you can use bluetooth.drawerPin5();
        } else {
          bluetooth.disconnect().whenComplete(() {
            print('disconnected inside print through bluetooth');
            printingOver = true;
            tappedPrintButton = false;
            setState(() {
              _connected = false;
...
//           });
    ...
    print('end of inside KotBytesPrintThroughBluetooth');
  }
  ...
  void _disconnectForKOTBytesBluetoothPrint() {
    print('came into disconnect');
    addRunningOrderToServer();
    Timer? _timer;
    int _everySecondForDisconnecting = 0;
    _everySecondForConnection = 0;
    _timer = Timer.periodic(const Duration(seconds: 1), (_) async {
      if (_everySecondForDisconnecting < 1) {
        print('timer disconnect is $_everySecondForDisconnecting');
        _everySecondForDisconnecting++;
      } else {
        bluetooth.disconnect().whenComplete(() {
