...
class StatisticsWithExpenses extends StatefulWidget {
  const StatisticsWithExpenses({Key? key}) : super(key: key);

  @override
  State<StatisticsWithExpenses> createState() => _StatisticsWithExpensesState();
}

class _StatisticsWithExpensesState extends State<StatisticsWithExpenses> {
  ...
  List<Query> queries = [];
  Map<int, dynamic> savedExpensesMonthData = HashMap();
  Map<int, dynamic> savedExpensesDayData = HashMap();
...
//ThisMeansWeHaveGotTheDataOfDayQueriesSinceItHasTheField-'Day'
          convertingDownloadedDaysExpenseToData(querySnapshot);
        } else {
...
//JustGivingSomeTimeForTheDataToBeAccumulatedAfterDownload
  ...
  void convertingDownloadedDaysExpenseToData(
      QuerySnapshot dayExpensesDataSnapshot) {
    for (var eachDayExpense in dayExpensesDataSnapshot.docs) {
      Map<String, dynamic> tempSavedDayBase = eachDayExpense['expenses'];
      int dayAdded = eachDayExpense['day'];
      int monthAdded = eachDayExpense['month'];
      int yearAdded = eachDayExpense['year'];
      int daysFromStartOfYear = DateTime(yearAdded, monthAdded, dayAdded)
              .difference(DateTime(yearAdded))
              .inDays +
          1;
      Map tempSavedExpensesData = HashMap();
      if (savedExpensesDayData[yearAdded] != null) {
        tempSavedExpensesData = savedExpensesDayData[yearAdded];
      }
      tempSavedExpensesData.addAll({daysFromStartOfYear: tempSavedDayBase});
      savedExpensesDayData.addAll({yearAdded: tempSavedExpensesData});
    }
  }

  void convertingDownloadedMonthsExpenseToData(
      QuerySnapshot monthsExpensesDataSnapshot) {
    for (var eachMonthExpense in monthsExpensesDataSnapshot.docs) {
      int monthAdded = eachMonthExpense['month'];
      int yearAdded = eachMonthExpense['year'];

      Map tempSavedExpensesData = HashMap();
      if (savedExpensesMonthData[yearAdded] != null) {
        tempSavedExpensesData = savedExpensesMonthData[yearAdded];
      }
      tempSavedExpensesData.addAll({monthAdded: eachMonthExpense['expenses']});
      savedExpensesMonthData.addAll({yearAdded: tempSavedExpensesData});
    }
  }
...
//ListOfAllItems
    ...
    if (calculationDaysDataRequired.isNotEmpty) {
      calculationDaysDataRequired.forEach((keyYear, valueArrayOfDays) {
        ...
        if (savedExpensesDayData.containsKey(keyYear)) {
          Map<dynamic, dynamic> tempEachYearData =
              savedExpensesDayData[keyYear];
...
//ThisMeansCompiledDataAlreadyExists
    ...
    if (calculationMonthsDataRequired.isNotEmpty) {
      calculationMonthsDataRequired.forEach((keyYear, valueArrayOfMonths) {
        ...
        if (savedExpensesMonthData.containsKey(keyYear)) {
          Map<dynamic, dynamic> tempEachYearData =
              savedExpensesMonthData[keyYear];
            ...
            children: [
              ...
              ListTile(
                ...
                trailing: ElevatedButton(
                    ...
                    onPressed: () {
                      statisticsReportsQueryGeneration();
                      viewClicked = true;
                      showSpinner = true;
                      setState(() {});
                    },
