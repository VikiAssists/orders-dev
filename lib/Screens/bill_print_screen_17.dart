...
class BillScreenWithNewBottomButton extends StatefulWidget {
  ...
  @override
  State<BillScreenWithNewBottomButton> createState() =>
      _BillScreenWithNewBottomButtonState();
}

class _BillScreenWithNewBottomButtonState
    extends State<BillScreenWithNewBottomButton> {
...
//booleanToUnderstandThatBillHasBeenUpdatedInServer
  ...
  Map<String, dynamic> tempCategoryAndItemsMap = HashMap();
...
//StartOfNewVariablesForNewPrinterPackage
  ...
  List<String> paymentMethodListVersion2 = [];
...
//AndAtTheSameTimeWeCanUseTheSameStreamToGetDataForPaymentDoneAlso
  ...
  String paymentDropDownOrButton = '';
...
//ThisMeansTheDayAlreadyExistsAndWeOnlyHaveToAddTheIncrementsToDayIncrements
              ...
              changeOfEachMonthCashBalanceData.addAll({
                dayAsString: {
                  'dayIncrements':
                      FieldValue.increment(dayIncrementWhileIterating)
                }
              });
            } else {
...
//So,WeHaveToAddFirstDocumentOurselvesForThatMonth
          ...
          tempEntireCashBalanceChangeSheet.addAll({
...
//ExpensesPaidByMethod
    ...
    paymentMethod.clear();
    paymentMethodListVersion2.clear();
    paymentMethodListVersion2.add('Choose Payment');
    ;
    paymentMethod.add('Choose Payment');
    paymentMethod.addAll(tempPaymentMethodList);
    paymentMethodListVersion2.addAll(tempPaymentMethodList);
    paymentMethodListVersion2.add('Split Payment');
    paymentMethodListVersion2.add('New Method');
    paymentMethod.add('New Method');
    paymentDropDownOrButton = 'Choose Payment';
    print('paymentMethod');
    print(paymentMethod);
...
//thisWillCheckWhetherSerialNumberIsNotThereAndWeHaven'tCalledStreamYet
      ...
      serialNumberStreamForPrintingOrClosing();
...
//ThisIsToCheckPaymentDoneMapIsAvailableOrNot
    ...
    itemsInOrderFromServerMap.forEach((key, value) {
      if (value['itemCancelled'] == 'false') {
        ...
        mapToAddIntoItems['itemCategory'] = value['itemCategory'];
...
//CNU:numberOfUnitsByCaptain
//CTA:totalAmountOfEachItem
              tempItemsForUserStatsMap[eachItem['item']] = {
                'CNU': tempItemsForUserStatsMap[eachItem['item']]['CNU'] +
                    eachItem['number'],
                'CTA': ((tempItemsForUserStatsMap[eachItem['item']]['CTA']) +
...
//ThisIsIfIncrementMapDoesntHaveTheItem
              tempItemsForUserStatsMap.addAll({
                eachItem['item']: {
                  'CNU': eachItem['number'],
                  'CTA': (eachItem['priceofeach'] * eachItem['number'])
                }
              });
            }
          }
...
//TTS:ticketsAndTableStats
//TI:numberOfTicketsTaken
//OT:numberOfOrdersTaken
//ITS:itemsOrderTakenStats
        key: {
          'TTS': {
            'TI': FieldValue.increment(numberOfTicketsTaken),
            'OT': FieldValue.increment(1),
          },
          'ITS': tempCaptainItemsSoldStatsMap
        }
      });
    });
...
//FirstWeMakeSimpleCalculationForEachTicket
          tempItemCategory = eachItem['itemCategory'];
...
//MakingMapThatSaysTheCategoryOfTheItemWithTime
      if (tempCategoryAndItemsMap.containsKey(tempItemCategory)) {
//WeAddTheCategoryAndInsideThatItemsWithTimeInWhichItWasAdded.
//ThisWillGiveAnIdeaThatWhichCategoryThatItemLastRemainsIn
        Map<String, int> tempTempItemCategory =
            tempCategoryAndItemsMap[tempItemCategory];
        tempTempItemCategory
            .addAll({distinctItemName: DateTime.now().microsecondsSinceEpoch});
        tempCategoryAndItemsMap[tempItemCategory] = tempTempItemCategory;
      } else {
        tempCategoryAndItemsMap.addAll({
          tempItemCategory: {
            distinctItemName: DateTime.now().microsecondsSinceEpoch
          }
        });
      }
...
//NT:numberOfTickets
        ...
        tempIndividualMenuItemSoldStatMap.addAll({
//NU:numberOfUnits
          'NU': FieldValue.increment(numberOfEachDistinctItemForAddingIntoList)
        });
      ...
      if (totalPriceOfEachDistinctItemForAddingIntoList != 0) {
        ...
        tempIndividualMenuItemSoldStatMap.addAll({
//TA:totalAmountOfEachItem
          'TA': FieldValue.increment(
              totalPriceOfEachDistinctItemForAddingIntoList)
        });
...
//ThisMeansThereIsCgstAndWeNeedToIncrementCgst
          tempIndividualMenuItemSoldStatMap.addAll({
//CG:cgstAmountOfEachItem
            'CG': FieldValue.increment(num.parse(
                ((totalPriceOfEachDistinctItemForAddingIntoList) *
                        (json.decode(
...
//ThisMeansThereIsNoCgstInTheHotel
          tempIndividualMenuItemSoldStatMap
              .addAll({'CG': FieldValue.increment(0)});
        }
        if (json.decode(Provider.of<PrinterAndOtherDetailsProvider>(context,
                    listen: false)
            ...
            0) {
//ThisMeansThereIsCgstAndWeNeedToIncrementCgst
//SG:sgstAmountOfEachItem
          tempIndividualMenuItemSoldStatMap.addAll({
            'SG': FieldValue.increment(num.parse(
                ((totalPriceOfEachDistinctItemForAddingIntoList) *
                        (json.decode(
                                Provider.of<PrinterAndOtherDetailsProvider>(
                                        context,
...
//ThisMeansThereIsNoSgstInTheHotel
          tempIndividualMenuItemSoldStatMap
              .addAll({'SG': FieldValue.increment(0)});
        }
...
//UE:numberOfUnitsOfEachExtraItems
//TE:totalAmountOfEachExtraItem
//CGE:cgstAmountOfEachExtraItem
//SGE:sgstAmountOfEachExtraItem
          key: {
            'UE': FieldValue.increment(1),
            'TE': FieldValue.increment(value),
            'CGE': json.decode(Provider.of<PrinterAndOtherDetailsProvider>(
                            context,
                            listen: false)
                        .restaurantInfoDataFromClass)['cgst'] ==
                    0
                ...
                : FieldValue.increment(num.parse(((value) *
                        (json.decode(
                            ...
                            100))
                    .toStringAsFixed(2))),
            'SGE': json.decode(Provider.of<PrinterAndOtherDetailsProvider>(
                            context,
                            listen: false)
                        .restaurantInfoDataFromClass)['sgst'] ==
                    0
...
//TAC-totalCancelledAmount
        key: {
          'NTC': FieldValue.increment(numberOfTimes.reduce((a, b) => a + b)),
          'NIC': FieldValue.increment(numberOfItems.reduce((a, b) => a + b)),
          'TAC': FieldValue.increment(totalAmount.reduce((a, b) => a + b)),
        }
      });
    });
...
//IICC-IndividualItemsCaptain/ChefCancelled
          key: {
            'TCC': FieldValue.increment(numberOfTimes.reduce((a, b) => a + b)),
            'NCC': FieldValue.increment(numberOfItems.reduce((a, b) => a + b)),
            'ACC': FieldValue.increment(totalAmount.reduce((a, b) => a + b)),
            'IICC': updateMapWithIncrements(tempIndividualItemsCancelled)
          }
        });
      });
    }
...
//IICR-IndividualItemsCaptain/ChefRejected
          key: {
            'TCR': FieldValue.increment(numberOfTimes.reduce((a, b) => a + b)),
            'NCR': FieldValue.increment(numberOfItems.reduce((a, b) => a + b)),
            'ACR': FieldValue.increment(totalAmount.reduce((a, b) => a + b)),
...
//CorrectionFor:
//InCaseDiscount/ExtraChargesWereAddedAfterPaymentMethodWasChosen,
//PaymentMethodStillHasTheOldValueInBill.ThisLoopWillCorrectIt
    if (bottomBarPaymentClosingMap.isNotEmpty &&
        bottomBarPaymentClosingMap.length == 1 &&
        paymentDropDownOrButton != 'Split Payment') {
      Map<int, dynamic> tempBottomBarPaymentClosingMap =
          bottomBarPaymentClosingMap;
      bottomBarPaymentClosingMap.forEach((key, value) {
        tempBottomBarPaymentClosingMap[key]['amountSentInThisPaymentMethod'] =
            totalBillWithTaxes().round();
        tempBottomBarPaymentClosingMap[key]
                ['amountSentInThisPaymentMethodInString'] =
            totalBillWithTaxes().round().toString();
      });
      bottomBarPaymentClosingMap = tempBottomBarPaymentClosingMap;
    }
  ...
  void screenExitErrorAlertDialogBox(
      bool callFromCloseButtonTrueElseFalse) async {
    showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: Center(
        ...
        actions: [
          ...
          ElevatedButton(
              ...
              onPressed: () {
                Navigator.pop(context);
                if (callFromCloseButtonTrueElseFalse) {
                  screenPopOutTimerAfterServerUpdate();
                } else {
                  Navigator.pop(context);
                }
...
//IfBillHadAlreadyBeenPrintedSerialNumberNeedNotBeAdded
    ...
    Map<String, dynamic> subMasterSalesIndividualItemsStatistics = HashMap();
    Map<String, dynamic> subMasterSalesGeneralStatistics = HashMap();
    Map<String, dynamic> subMasterSalesCaptainStatistics = HashMap();
    subMasterSalesCaptainStatistics.addAll({
      'mapEachCaptainOrdersTakenStatsMap': mapEachCaptainOrdersTakenStatsMap
    });
    subMasterSalesGeneralStatistics
        .addAll({'mapGeneralStatsMap': generalStatsMap});

    subMasterSalesIndividualItemsStatistics
        .addAll({'menuIndividualItemsStatsMap': menuIndividualItemsStatsMap});
    if (extraIndividualItemsStatsMap.isNotEmpty) {
      subMasterSalesIndividualItemsStatistics.addAll(
          {'mapExtraIndividualItemsStatsMap': extraIndividualItemsStatsMap});
    }

    subMasterSalesIndividualItemsStatistics
        .addAll({'mapCategoryAndItemsMap': tempCategoryAndItemsMap});
    ...
    Map<String, dynamic> statisticsDailyIndividualItemsStatsMap = {
      'day': num.parse(statisticsDay),
      'month': num.parse(statisticsMonth),
      'year': num.parse(statisticsYear)
    };
    Map<String, dynamic> statisticsMonthlyIndividualItemsStatsMap = {
      'month': num.parse(statisticsMonth),
      'year': num.parse(statisticsYear),
      'midMonthMilliSecond':
          DateTime(int.parse(statisticsYear), int.parse(statisticsMonth), 15)
    ...
    Map<String, dynamic> statisticsDailyCaptainStatsMap = {
      'day': num.parse(statisticsDay),
      'month': num.parse(statisticsMonth),
      'year': num.parse(statisticsYear)
    };
    Map<String, dynamic> statisticsMonthlyCaptainStatsMap = {
      'month': num.parse(statisticsMonth),
      'year': num.parse(statisticsYear),
      'midMonthMilliSecond':
          DateTime(int.parse(statisticsYear), int.parse(statisticsMonth), 15)
    ...
    statisticsDailyCaptainStatsMap.addAll(subMasterSalesCaptainStatistics);
    statisticsMonthlyCaptainStatsMap.addAll(subMasterSalesCaptainStatistics);
    ...
    statisticsDailyGeneralStatsMap.addAll(subMasterSalesGeneralStatistics);
    statisticsMonthlyGeneralStatsMap.addAll(subMasterSalesGeneralStatistics);
    statisticsDailyGeneralStatsMap
        .addAll({'cashierClosingSalesStats': finalCashierClosingMap});
    statisticsMonthlyGeneralStatsMap
        .addAll({'cashierClosingSalesStats': finalCashierClosingMap});
    statisticsDailyGeneralStatsMap
        .addAll({'paymentMethodClosingSalesStats': finalPaymentClosingMap});
    statisticsMonthlyGeneralStatsMap
        .addAll({'paymentMethodClosingSalesStats': finalPaymentClosingMap});

    statisticsDailyIndividualItemsStatsMap
        .addAll({'salesIncomeStats': subMasterSalesIndividualItemsStatistics});
    statisticsMonthlyIndividualItemsStatsMap
        .addAll({'salesIncomeStats': subMasterSalesIndividualItemsStatistics});
    ...
    if (billUpdatedInServer == false) {
      ...
      FireStoreBillAndStatisticsInServerVersionThree(
              ...
              printOrdersMap: updatePrintOrdersMap,
              orderHistoryDocID: orderHistoryDocID,
              dailyGeneralStatisticsUpdateMap: statisticsDailyGeneralStatsMap,
              monthlyGeneralStatisticsUpdateMap:
                  statisticsMonthlyGeneralStatsMap,
              dailyCaptainStatisticsUpdateMap: statisticsDailyCaptainStatsMap,
              monthlyCaptainStatisticsUpdateMap:
                  statisticsMonthlyCaptainStatsMap,
              dailyIndividualItemsStatisticsUpdateMap:
                  statisticsDailyIndividualItemsStatsMap,
              monthlyIndividualItemsStatisticsUpdateMap:
                  statisticsMonthlyIndividualItemsStatsMap,
              entireCashBalanceChangeSheet: entireCashBalanceChangeSheet,
              versionOneStatisticsUpdateMap: statisticsMap,
              year: statisticsYear,
              month: statisticsMonth,
              day: statisticsDay)
          .updateBillAndStatistics();
...
//WeNeedStopThePaymentDoneIn5Seconds
    if (serialNumber == 0) {
      ...
      FirebaseFirestore.instance
          ...
          .doc(statisticsYear)
          .collection(statisticsMonth)
          .doc('$statisticsDay*generalStats')
          .set({'statisticsDocumentIdMap': tempSerialInStatisticsMap},
              SetOptions(merge: true));
    }
    clearingCurrentUserBillingTime();
                  ...
                  child: Column(
                    ...
                    children: bottomBarPaymentClosingMap.entries.map((entry) {
                      ...
                      TextEditingController newPaymentMethodController =
                          TextEditingController(text: tempNewPaymentMethod);
                      newPaymentMethodController.selection =
                          TextSelection.collapsed(
                              offset: tempNewPaymentMethod.length);
                              ...
                              child: Center(
                                child: DropdownButtonFormField(
                                  ...
                                  value: paymentMethodValueMap[
                                      'chosenPaymentMethod'],
                                  onChanged: (value) {
                                    if (bottomBarPaymentClosingMap.length > 1) {
                                      setState(() {
                                        paymentDropDownOrButton =
                                            'Split Payment';
                                      });
                                    }
                                    if (bottomBarPaymentClosingMap.length ==
                                            1 &&
                                        paymentDropDownOrButton !=
                                            'Split Payment') {
                                      setState(() {
                          ...
                          Visibility(
                              ...
                              child: Row(
                                ...
                                children: [
                                  SizedBox(width: 10),
                                  Text(
                                    'Please Enter Amount paid in this Payment Method',
...
//ThisWillCalculateAllThePaymentsThatHasBeenEnteredTillNow
                                      ...
                                      if (totalAmountFromAllPaymentMethods >
                                        ...
                                        errorAlertDialogBox();
                                          ...
                                          totalBillWithTaxes().round()) {
                                        errorMessage =
                                            'Reduce Amount in this payment method to split further';
                                        errorAlertDialogBox();
                                      } else {
                                      ...
                                      child: Padding(
                                        ...
                                        child: BottomButton(
                                            onTap: () async {
                                              print(
                                                  'bottomBarPaymentClosingMap');
                                              print(bottomBarPaymentClosingMap);
                                              bool hasInternet =
...
//ToAddNewPaymentMethodInServer
                                                  ...
                                                  tempFinalPaymentClosingMap
                                                    ...
                                                    finalPaymentClosingMap
                                                        .addAll({
                                                      key: {
//TAPM:totalAmountInThisPaymentMethod
//NTPM:numberOfTimesInThisPaymentMethod
                                                        'TAPM': FieldValue
                                                            .increment(value[
                                                                'totalAmount']),
                                                        'NTPM': FieldValue
...
//ThisIsFinalClosingMapForCashierAddingThePaymentMethodsForThisCashier
                                                  ...
                                                  tempFinalCashierClosingMap
                                                      .addAll({
//NOC:numberOfOrdersClosed
//TAC:totalAmountClosed
                                                    'NOC':
...
//TheDocumentShouldBePreviousDayInTheBilledDate
  ...
  Future<void> serialNumberStreamForPrintingOrClosing() async {
          ...
          if (serialNumber == 0 && !noItemsInTable) {
            if (gotSerialNumber == false) {
              ...
              if (statisticsData!.containsKey('mapGeneralStatsMap')) {
                tempGeneralStats = statisticsData['mapGeneralStatsMap'];
              }

              if (tempGeneralStats.isEmpty ||
                  !tempGeneralStats.containsKey('serialNumber')) {
                serialNumber = 1 + numberOfOrdersBeforeThisTable;
              } else {
                serialNumber =
                    num.parse((tempGeneralStats['serialNumber']).toString())
                            .toInt() +
                        1 +
                        numberOfOrdersBeforeThisTable;
              }
    ...
    return WillPopScope(
      onWillPop: () async {
        if (billUpdatedInServer || noItemsInTable) {
          int count = 0;
          Navigator.of(context).popUntil((_) => count++ >= 2);
        } else if (bottomBarPaymentClosingMap.isNotEmpty) {
          errorMessage =
              'Payment Method Chosen will be deleted. Do you want to exit?';
          screenExitErrorAlertDialogBox(false);
        } else {
          Navigator.pop(context);
        }
        return false;
            ...
            IconButton(
                onPressed: () {
                  if (bottomBarPaymentClosingMap.isNotEmpty) {
                    errorMessage =
                        'Payment Method Chosen will be deleted. Do you want to exit?';
                    screenExitErrorAlertDialogBox(true);
                  } else {
                    screenPopOutTimerAfterServerUpdate();
                  }
                },
                                                            ...
                                                            child: (title !=
                                                                ...
                                                                : Text(title,
                                                                    style: const TextStyle(
                                                                        ...
                                                                        fontWeight:
                                                                            FontWeight.bold)),
//ThisWillEnsureSplitPaymentAndNewMethodAreInDifferentColor
                                                            value: title,
                                                          );
                                                        }).toList(),
                                                      ),
                                                                  ...
                                                                  children: [
                                                                    Expanded(
                                                                      ...
                                                                      child: ElevatedButton(
                                                                          ...
                                                                          onPressed: () {
                                                                            setStateSB(() {
                                                                              tempDiscountValueClickedTruePercentageClickedFalse = false;
                                                                    ...
                                                                    Expanded(
                                                                        ...
                                                                        child: ElevatedButton(
                                                                            style: ButtonStyle(
                                                                              backgroundColor: tempDiscountValueClickedTruePercentageClickedFalse ? MaterialStateProperty.all(Colors.green) : MaterialStateProperty.all(Colors.grey),
                                                                            ),
                                                                            onPressed: () {
                                                                              setStateSB(() {
                                                                                tempDiscountValueClickedTruePercentageClickedFalse = true;
                                                                                tempDiscountEnteredValue = '';
                                                                                _controller.clear();
                                                                              });
                                                                            },
                                                                  ...
                                                                  child:
                                                                    ...
                                                                    decoration:
                                                                        ...
                                                                        InputDecoration(
                                                                            ...
                                                                            hintStyle:
                                                                                TextStyle(color: Colors.grey),
                                                                            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(10)), borderSide: BorderSide(color: Colors.green)),
                                                                            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(10)), borderSide: BorderSide(color: Colors.green))),
                                                                  ),
                                                                ),
                                                                ElevatedButton(
                                                                    style:
                                          ...
                                          child: Expanded(
                                            child: BottomButton(
                                              onTap: () async {
                                                if (bottomBarPaymentClosingMap
                                                    .isEmpty) {
//ThisMeansTheyGavePaymentDoneWithoutChoosingPaymentMethod
                                                                ...
                                                                .set(
                                                                  ...
                                                                  'statisticsDocumentIdMap':
                                                                    ...
                                                                    SetOptions(
                                                                        merge:
                                                                            true));
//PuttingInBothStatisticsSoThatItWillHaveSerialNumberRight
                                                            setState(() {
                                                              showSpinner =
                                                                  true;
                                                            });
