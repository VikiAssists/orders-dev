...
class ExpensesSeparateFolder extends StatefulWidget {
  const ExpensesSeparateFolder({Key? key}) : super(key: key);

  @override
  State<ExpensesSeparateFolder> createState() => _ExpensesSeparateFolderState();
}

class _ExpensesSeparateFolderState extends State<ExpensesSeparateFolder> {
...
//DeletingAnExistingCategory/Vendor/PaidBy/PaymentMethod
...
//EditingAnExistingCategory/Vendor/PaidBy/PaymentMethod
...
//ThisMeansWeCanUpdateIncrementsAsBatch
        ...
        entireCashBalanceChangeSheet.forEach((key, value) {
          ...
          num midMonthMilliSecond =
              DateTime(int.parse(splitKeyForYearMonth[0]), int.parse(splitKeyForYearMonth[1]), 15)
                  .millisecondsSinceEpoch;
          ...
          batch.set(statisticsMonthlyReportForExpensesRef,
              {'cashBalanceData': value,'midMonthMilliSecond':midMonthMilliSecond,
        ...
        entireCashBalanceChangeSheet.forEach((key, value) {
          ...
          batch.set(statisticsMonthlyReportForExpensesRef,
              {'cashBalanceData': value,
                'midMonthMilliSecond':midMonthMilliSecond,
                'year': int.parse(splitKeyForYearMonth[0]),
                'month': int.parse(splitKeyForYearMonth[1])}, SetOptions(merge: true));
        });
        batch.commit();
        _streamSubscriptionForThisMonth?.cancel();
        currentGeneratedIncrementRandomNumber = 0;
...
//NEC:NumberOfEntriesOfCategory
//NUC:numberOfUnits
//CGC:cgstValueOfThisCategory
//SGC:sgstValueOfThisCategory
//TPC:totalPriceOFThisCategory
          'NEC': FieldValue.increment(1),
          'NUC': tempNumberOfUnitsInString != ''
              ? FieldValue.increment(num.parse(tempNumberOfUnitsInString))
              : FieldValue.increment(0),
          'CGC': tempCgstValueOfItemInString != ''
              ? FieldValue.increment(num.parse(tempCgstValueOfItemInString))
              : FieldValue.increment(0),
          'SGC': tempSgstValueOfItemInString != ''
              ? FieldValue.increment(num.parse(tempSgstValueOfItemInString))
              : FieldValue.increment(0),
          'TPC': tempExpenseOrIncome == 'Expense'
...
//NEUP:numberOfEntriesOfUserPaying
//UPA:UserPaidAmount
          'NEUP': FieldValue.increment(1),
          'UPA': tempExpenseOrIncome == 'Expense'
              ? FieldValue.increment(
              num.parse(tempTotalExpenseWithTaxesInString))
              : FieldValue.increment(
              num.parse(tempTotalExpenseWithTaxesInString) * -1),
...
//NEPM:numberOfEntriesInPaymentMethod
//PAPM:paidAmountInPaymentMethod
              'NEPM': FieldValue.increment(1),
              'PAPM': tempExpenseOrIncome == 'Expense'
                  ? FieldValue.increment(
                  num.parse(tempTotalExpenseWithTaxesInString))
                  : FieldValue.increment(
                  num.parse(tempTotalExpenseWithTaxesInString) * -1)
...
//UPA:UserPaidAmount
          'NEUP': FieldValue.increment(-1),
          'UPA': beforeEditExpenseOrIncome == 'Expense'
              ? FieldValue.increment(
              -1 * (num.parse(beforeEditTotalPriceWithTaxes)))
              : FieldValue.increment(num.parse(beforeEditTotalPriceWithTaxes)),
          'paymentMethod': {
...
//PAPM:paidAmountInPaymentMethod
              'NEPM': FieldValue.increment(-1),
              'PAPM': beforeEditExpenseOrIncome == 'Expense'
                  ? FieldValue.increment(
                  -1 * (num.parse(beforeEditTotalPriceWithTaxes)))
                  : FieldValue.increment(num.parse(beforeEditTotalPriceWithTaxes))
            }
...
//PAPM:paidAmountInPaymentMethod
      ...
      Map<String, dynamic> statisticsMonthlyExpensesMap = {
        ...
        'midMonthMilliSecond': DateTime(
            int.parse(year),int.parse(month),15
        ).millisecondsSinceEpoch
      };
...
//UPA:UserPaidAmount
        beforeEditExpensePaidByUser: {
          ...
          'UPA': beforeEditExpenseOrIncome == 'Expense'
              ...
              : FieldValue.increment(
              num.parse(beforeEditTotalPriceWithTaxes)),
          'paymentMethod':{beforeEditPaymentMethod: {
//NEPM:numberOfEntriesInPaymentMethod
//PAPM:paidAmountInPaymentMethod
            'NEPM':
            FieldValue.increment(-1),
